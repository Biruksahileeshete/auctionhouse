...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      // Decode just enough to know our own user id for outbid detection —
      // JWT payload is base64, no need for a full library for this.
      try {
    ...
    socket.on("new-bid", (data: { listingId: string; amount: number; bidderId: string; endsAt: string; wasExtended: boolean }) => {
      ...
      // Refetch to get the updated bid list with bidder names attached.
